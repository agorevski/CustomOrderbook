...
class OrderBookDeployer:
    ...
    def _setup_web3(self) -> Web3:
        ...
        if not w3.is_connected():
            raise ConnectionError(f"Failed to connect to {self.network} network at {rpc_url}")
    ...
    def compile_contract(self, node_modules_dir: str) -> Dict[str, Any]:
        ...
        if not contract_path.exists():
            raise FileNotFoundError("OrderBook.sol not found in src/contracts directory")
    ...
    def run(self, node_modules_dir: str):
        ...
        try:
            ...
            # Verify
            verification_success, owner, next_order_id, verification_status = self.verify_deployment(
                contract_address, 
                contract_data['abi']
            )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--network', type=str, default='tenderly', help='Network to deploy to (default: tenderly)')
    parser.add_argument('--node-modules-dir', type=str, default='C:\\GIT\\node_modules', help='Path to node_modules directory (if needed)')
