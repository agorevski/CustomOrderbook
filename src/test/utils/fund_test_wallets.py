...
def fund_native_balance(rpc_url, addresses, amount_wei):
    ...
    print_info(f"Funding {len(addresses)} address(es) with {Web3.from_wei(amount_wei, 'ether')} ETH each...")
...
def fund_erc20_balance(rpc_url, token_address, addresses, amount):
    ...
    print_info(f"Funding {len(addresses)} address(es) with {amount} tokens from {token_address}...")
    ...
    result = tenderly_rpc_call(
        ...
        [token_address, addresses, amount_hex]
    )
...
def main():
    ...
    print_info(f"Amount to fund: {ask_amount_base} tokens = {ask_amount_with_decimals} (with decimals)")
    
    fund_erc20_balance(
        rpc_url,
        ask_token_address,
        [ask_wallet['address']],
        ask_amount_with_decimals
    )
    ...
    print_info(f"Amount to fund: {fill_amount_base} tokens = {fill_amount_with_decimals} (with decimals)")
    
    fund_erc20_balance(
        rpc_url,
        fill_token_address,
        [fill_wallet['address']],
        fill_amount_with_decimals
    )
